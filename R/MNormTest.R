#' @title MNormTest
#' @description This package provides functions to test the mean vector and covariance matrix of multivariate normal distribution.
#' @author Xifeng Zhang

#' Test of Single Mean Vector
...
mean.test.multi <- function(X, label, alpha = 0.05) {
    ...
    Within.mean <- lapply(data.split, function(x) {apply(x, 2, mean)})
    At <- lapply(data.split, function(x) {(nrow(x)-1) * cov(x)})
    A <- Reduce("+", At)
    ...
    Reject <- c("Bartlett" = ifelse(p.value.chi2 < alpha, "Reject", "Not Reject"),
...
#' Test of Single Covariance Matrix
#' @param data The data matrix which is a matrix or data frame.
...
#' Test of Multiple Covariance Matrix
#' @param X The data matrix which is a matrix or data frame.
#' @param label A vector of group labels.
#' @param alpha The significance level. Default is 0.05.
...
cov.test.multi <- function(X, label, alpha = 0.05) {
    ...
    St <- lapply(data.split, function(x) {cov(x)})
    St.root <- lapply(data.split, function(x) {mpfr(det(cov(x)), 256)^(-(nrow(x)-1)/2)})
    St.sum <- lapply(data.split, function(x) {(nrow(x)-1) * log(mpfr(det(cov(x)), 256))})
    St.star <- lapply(data.split, function(x) {cov(x) * (nrow(x) - 1) / nrow(x)})
    St.star.root <- lapply(data.split, function(x) {mpfr(det(cov(x) * (nrow(x) - 1) / nrow(x)), 256)^(-nrow(x)/2)})
    ...
    statistics <- data.frame("Statistics" = c("Likelihood Ratio", "Modified Likelihood Ratio", "M", "Chi2"),
...
#' Test of Mean and Covariance Matrix at the Same Time
#' @param X The data matrix which is a matrix or data frame.
#' @param label A vector of group labels.
#' @param alpha The significance level. Default is 0.05.
...
MeanCov.test <- function(X, label, alpha = 0.05) {
    ...
    lambda <- Reduce("*", lapply(data.split, function(x) {mpfr(det((nrow(x)-1) * cov(x)), 256)^(nrow(x)/2)})) / mpfr(det(T), 256)^(n/2) * n^(n*p/2) / mpfr(Reduce("*", lapply(data.split, function(x) {nrow(x)^(nrow(x)*p/2)})), 256)

    lambda.star <- Reduce("*", lapply(data.split, function(x) {mpfr(det((nrow(x)-1) * cov(x)), 256)^((nrow(x)-1)/2)})) / mpfr(det(T), 256)^((n-k)/2) * (n-k)^((n-k)*p/2) / mpfr(Reduce("*", lapply(data.split, function(x) {(nrow(x)-1)^((nrow(x)-1)*p/2)})), 256)

    d <- (Reduce("+", lapply(data.split, function(x) {sum(1/(nrow(x)-1))})) - sum(1/(n-k))) * ((2*p^2+3*p-1) / (6*(p+3)*(k-1))) - (p - k + 2) / ((n-k)*(p+3))
